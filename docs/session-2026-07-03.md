...
## OOM: modalità sicura per `R/base/base_sequential_analysis.R`

- Gli OOM storici avvengono durante `mclapply`/`%dopar%` delle proiezioni: Slurm uccide il job, non Leonardo.
- Con più worker il fork usa inizialmente copy-on-write, ma cache Terra/GDAL, modelli e buffer di predizione diventano memoria privata per worker.
- Per una proiezione realmente sequenziale e disk-backed:
...
- `BIOMOD_NCPU=1`: nessun worker parallelo; `keep.in.memory=false`: non trattiene lo stack in R; `do.stack=false`: scrive una proiezione per modello su disco.
- Non esiste un'attesa affidabile della RAM: al limite del cgroup Slurm il kernel uccide il processo. Questa modalità riduce il picco, ma un singolo modello oltre la RAM disponibile può comunque fallire.
