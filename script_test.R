...
# loading CURRENT environmental data
#####################################
clim_proj=rast(dir("HPC_Leonardo/Var_Climate/Baseline", full.names=T))
tri_proj=rast(dir("HPC_Leonardo/Var_TRI", full.names=T))
soil_proj=rast(dir("HPC_Leonardo/Var_Soil", full.names=T))
...
lf=list.dirs("HPC_Leonardo/Var_Climate/future", full.names=T, recursive = T)[-1]
lf<-as.matrix(lf)
lf<-lf[nchar(lf[,1]) >= 33, ]  # TODO refactor this line programmatically
#####################################
